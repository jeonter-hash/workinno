...
const T = deck.T, C = deck.C, BT = deck.bt;
...
/* 3 핵심 요약 */
{
  ...
  B.head(s, { title:'핵심 요약',
    message:'절감 여력 8,100시간 중 65%가 상위 4개 업무에 집중되어 있어, 선별 추진 시 1년 내 6,300시간 절감이 가능함' });
  ...
  [['01  진단',['12개 업무의 시간을 실측','절감 경로 3대 유형을 확인']],
  ...
  B.footnote(s,'주) 절감 시간은 12개 업무의 실측값이며, 인건비 환산액은 포함하지 않음',{ x:M, y:BB-0.24, w:CW });
}

/* 4 워터폴 */
{
  const s=S('01 현황 진단');
  B.head(s, { title:'절감 경로별 기여도',
    message:'4개 경로 중 문서 자동화가 21천 시간으로 최대 기여하며, 4개 합산 시 목표 72천 시간 수준에 도달함' });
...
{
  const s=S('01 현황 진단');
  B.head(s, { title:'절감 시간의 3대 경로',
    message:'절감 시간은 자동화·표준화·이관 3개 경로로 구성되며, 해당 경로 외 활동은 절감 산정에서 제외함' });
  ...
  ['자동화: 검증된 도구를 적용하므로 3개월 내 완료 가능','표준화: 부서 합의가 병목이므로 게이트로 구간을 끊어 관리','이관: 교육을 병행하지 않으면 종전 방식으로 원복될 우려']
   .forEach((t,i)=>{
    const y=BT+i*1.62;
...
{
  const s=S('02 전환 과제');
  B.head(s, { title:'5단계 게이트와 통과 기준',
    message:'단계별 산출물을 다음 단계 입력으로 연결하며, 게이트 미통과 시 다음 단계 진입을 유보함' });
...
{
  const s=S('03 실행 로드맵');
  B.head(s, { title:'우선 착수 4개 과제',
    message:'효과 대·난이도 소 영역의 4개 과제를 1차 추진하고, 우측 하단 영역은 당해연도 착수 대상에서 제외함' });
...
{
  const s=S('04 성과 관리');
  B.head(s, { title:'3단 교육 체계',
    message:'전 직원 공통 과정을 기반으로 직무별 심화와 추진단 전문 과정을 단계적으로 구성함' });
...
{
  const s=S('04 성과 관리');
  B.head(s, { title:'성과지표 5종',
    message:'선행지표(이관율·수료율)와 결과지표(절감시간·오류율)를 분리하여 관리하고 측정 주기를 사전 확정함' });
  B.kpiRow(s,[{v:'6,300h',l:'연간 절감 목표'},{v:'90%',l:'업무 이관율'},
              {v:'-40%',l:'처리 오류율'},{v:'92%',l:'교육 수료율',dark:true}],{ y:BT, h:1.15 });
  B.sectionTitle(s,'선행지표의 현재와 목표 (%)',{ x:M, y:BT+1.31, w:CW, sz:12 });
  B.barChart(s,{ x:M, y:BT+1.72, w:CW, h:2.32, horizontal:true, max:100,
    cats:['업무 이관율','교육 수료율','표준 준수율','자동화 적용률'],
    series:[{ name:'현재', values:[12,35,48,22] }, { name:'목표', values:[90,92,85,70] }] });
  B.callout(s,'선행지표가 2회 연속 목표에 미달하는 경우 해당 과제를 재설계함',{ x:M, y:BB-0.58, w:CW, h:0.58 });
}

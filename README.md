...
### brandlogy-pptx — Brandlogy 브랜드 PPT (16:9 디자인 시스템)

`.claude/skills/brandlogy-pptx/`

순백 캔버스 · Pretendard 전용 · 브랜드 블루(#1456f0) 체계의 16:9 브랜드 덱을 만드는 스킬. 5존 고정 좌표(헤더 0.4" / 헤드라인 1.0" / 부제 1.63" / 본문 2.39–6.85" / 푸터 7.05")와 Hero Gradient 상한, 로고 무결성, 시각화 우선 규칙을 코드로 강제한다.
...
| `references/layout-geometry.md` | 인치 확정 좌표표 — 5존, 12열 그리드, 패턴 A~F 밴드, 그림자·그라디언트 OOXML, 차트 기본값 |
...
| `scripts/apply_gradient.py` | Hero Gradient 센티넬 → 벡터 `a:gradFill` 후처리 (장표 1 / 덱 3 상한 검사) |
| `scripts/check_layout.py` | 체크리스트 자동 점검 — 존 앵커·하드 경계·Pretendard·밀도·로고 뒤 도형·이모지·팔레트 |

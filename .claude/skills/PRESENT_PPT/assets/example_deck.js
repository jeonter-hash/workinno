...
/* 1 표지 · 2 목차 */
B.cover(deck, { title:'AX 전환 로드맵', subtitle:'업무 재설계부터 인력 전환까지 12개월 실행 설계',
  org:'한국표준협회', team:'전환추진단', date:'2026-01-15' });
B.toc(deck, { items:[
...
/* 3 핵심 요약 */
{
  ...
  ['진단 — 12개 업무 실측, 절감 경로 3종 확인','설계 — 5단계 게이트, 통과 기준 사전 합의',
   '실행 — 4개 과제 우선, 분기별 산출물 1건'].forEach((t,i)=>{
    const y=BT+1.52;
    B.box(s,{ x:cells[i].x, y, w:cells[i].w, h:1.72, fill:K.w, line:K.g4 });
    B.txt(s,`0${i+1}`,{ x:cells[i].x+T.pad, y:y+0.16, w:0.7, h:0.30, sz:12, b:true, c:K.g2 });
    B.txt(s,t,{ x:cells[i].x+T.pad, y:y+0.56, w:cells[i].w-2*T.pad, h:1.00, sz:T.body, b:true, lh:1.4 });
  });
  B.callout(s,'권고 — 상위 4개 과제에 자원을 집중하고, 잔여 과제는 2차 연도로 이월',{ x:M, y:BT+3.44, w:CW, h:0.72, dark:true });
  B.footnote(s,'※ 절감 시간은 실측 기준이며 인건비 환산은 별도 산정 필요',{ x:M, y:BB-0.24, w:CW });
}
...
/* 11 성과 관리 */
{
  ...
  B.tableNative(s,{ x:M, y:BT+1.36, colW:[2.6,1.5,1.5,1.5,2.7333], headH:0.38, rowH:0.44,
    ...
    rows:[['업무시간 절감','결과','0h','6,300h','업무 로그 집계'],
          ...
          ['표준 준수율','선행','—','85%','분기 감사']] });
  B.callout(s,'선행지표가 2개 분기 연속 미달할 경우 과제 재설계 절차 착수',{ x:M, y:BB-0.58, w:CW, h:0.58 });
}

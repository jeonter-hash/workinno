...
- **Logo**: 한국표준협회(KSA) 로고만 사용한다. MiniMax·Brandlogy를 비롯한 어떤 제3자 로고도 대체 사용 금지. Place at top-right of every slide unless explicitly told otherwise. Use the logo file exactly as provided — see Logo Integrity Rule below. 로고 원본은 `assets/ksa_logo.jpg`(사용자 제공 "Future Value, Ask KSA" 워드마크, 325×42px, **흰 배경 JPEG**)에 있고 헬퍼가 기본값으로 쓴다. 흰 배경 장표에서만 안전하며, 어두운 배경·그라디언트 장표용 투명/흰 반전본은 사용자에게 받는다 — 직접 그리거나 재현하지 않는다.
